...
- **First source-diversity intervention (2026-09-06):** the current snapshot has 137 stories—135 ynet and two BBC News. The updater now scans 100 rather than 25 entries per feed, an official Guardian Israel feed is enabled, and the non-contributing legacy Reuters feed is disabled but retained in the registry for traceability. The UI exposes current per-source counts and leading-source share. Network-wide DNS failure prevented a local live-feed verification, so improved production diversity remains an outcome to verify over subsequent scheduled runs rather than a completed claim.
