...
- **Weak normalization:** deduplication is exact URL only; generated IDs are truncated base64; feeds can produce URL variants and syndication duplicates. Entity arrays and clusters are empty in the audited dataset, so party/person filtering and comparison are mostly scaffolding.
...
- **Validation/testing gaps:** validation checks only presence of a few story fields and URL parsing. It does not enforce schemas/types, source consistency, unique IDs/URLs, poll shape, timestamps, safe protocols, excerpt length/label, health data, or cross-file relationships. There are no component/end-to-end tests.
- **Reproducibility/maintenance debt:** dependencies use `latest`, no root `package-lock.json` is committed, and workflows use `npm install`; future builds can change without a code change. Actions are version tags rather than pinned commits. Source, entity, methodology, and incident ownership are undocumented.
- **Workflow/data risks:** scheduled runs commit directly to the checked-out branch with write permission and push without explicit concurrency/rebase handling. Deployment has overlapping triggers and no end-to-end post-deploy health check or documented rollback. Routine generated commits can obscure operational failures and complicate concurrent human pushes.
